...
"""
from transformers import Trainer, logging, TrainerState
...
###############################################
# Minimal HuggingFace Trainer override example
###############################################
class MetaTrainer(Trainer):
  ...
  Key Points:
    - create_optimizer(): build meta-optimizer
    - training_step(): normal forward/backward pass + store data for meta closure
    - _inner_training_loop(): simple patch of trainer so that optimizer.step() is called with the closure
  """

  def create_optimizer(self):
    """
    Called by Trainer when setting up the optimizer & OPTIONAL scheduler
    """

    if self.optimizer is not None:
      logger.info("[Using existing Meta Optimizer]")
      return self.optimizer

    # Read meta_optimizer_params from the instance
    self.optimizer = MetaOpt(
            model=self.model,
            H=self.meta_optimizer_params.get("H", 5),
            HH=self.meta_optimizer_params.get("HH", 10),
            m_method=self.meta_optimizer_params.get("m_method", "scalar"),
            base_lr=self.meta_optimizer_params.get("base_lr", lambda t: 1e-3),
    ...
    logger.info(f"\nH={self.optimizer.H}, HH={self.optimizer.HH}, m_method={self.optimizer.m_method}")
    logger.info(f"\nbase_lr={self.optimizer.base_lr}, weight_decay={self.optimizer.weight_decay}")
  ...
  ##################################[META-ADDED]#########################################
  # Functionality remains basically the same as default HF training_step(), with the addition of building a closure for optimizer.step()
  def training_step(self, model: nn.Module, inputs: dict[str, Union[torch.Tensor, Any]], num_items_in_batch=None) -> torch.Tensor:
          """
          Runs forward + backward pass and prepares the closure for MetaOpt.
          """
          model.train()
          ...
          def cost_fn(model, param_info, flat_params, inps):
            param_dict = unflatten_params(flat_params, param_info)
            device = next(model.parameters()).device
            inputs = {k: v.to(device=device) for k, v in inps.items()}
            outputs = functional_call(module=model, args=(), parameter_and_buffer_dicts=param_dict, kwargs=inputs)
            return outputs.loss

          # Store data for use in closure passed to optimizer.step
          self._meta_data = (inputs, cost_fn)
  ...
  # Slighlty modified to pass the closure to the optimizer.step() call
            ...
            for _ in range(total_updates):
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    if do_sync_step:
                        ...
                        ##################################[META-ADDED]#########################################
                        # CALLING THE OPTIMIZER WITH THE METADATA CREATED IN SELF.TRAINING_STEP
                        self.optimizer.step(closure=self._meta_data)
                        #######################################################################################
